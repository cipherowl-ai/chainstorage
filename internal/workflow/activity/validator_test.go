...
func TestGetValidationEndHeight(t *testing.T) {
	tests := []struct {
		name                                      string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := getValidationEndHeight(tt.start, &api.BlockMetadata{Height: tt.persisted}, tt.tip, request, zap.NewNop())
			require.NoError(err)
